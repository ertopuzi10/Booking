...
namespace Booking.Application.Features.Bookings.Commands.CreateBooking
{
    public class CreateBookingCommandHandler : IRequestHandler<CreateBookingCommand, Guid>
    {
        ...
        public async Task<Guid> Handle(CreateBookingCommand request, CancellationToken cancellationToken)
        {
            ...
            var guest = await _context.UsersQuery
                .FirstOrDefaultAsync(u => u.Id == _currentUserService.UserId.Value, cancellationToken);

            if (guest == null || guest.IsSuspended)
                throw new UnauthorizedAccessException("Your account is suspended. You cannot create bookings.");
            ...
            if (host != null)
            {
                var guestUser = await _context.UsersQuery
                    .FirstOrDefaultAsync(u => u.Id == booking.GuestId, cancellationToken);

                await _emailService.SendNewBookingRequestAsync(
                    host.Email,
                    host.FirstName,
                    guestUser != null ? $"{guestUser.FirstName} {guestUser.LastName}" : "A guest",
                    property.Name,
                    booking.StartDate,
                    booking.EndDate);
            }
